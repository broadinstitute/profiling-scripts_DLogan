...
python -m cpa.profiling.profile_mean -o ${DDIR}/PerWell_${DATESTAMP}_${DBROOT}_MEAN+STD_DMSO.csv -c -f DMSO --lsf-directory=${DDIR}/.compute_2c --njobs=250 --normalization=DummyNormalization --method=mean+std ${PROPDIR}/${PROP_FILE} ${DDIR}/${CACHE_DIR} ${GRPCOL}
## From 2013_02_19
#python -m cpa.profiling.profile_mean -o ${DDIR}/PerWell_AVG+STD_DMSO.csv -c -f Con --lsf-directory=.compute --njobs=250 --normalization=DummyNormalization --method=mean+std ${DDIR}/${PROP_FILE} ${DDIR}/${CACHE_DIR} Plate_Well
